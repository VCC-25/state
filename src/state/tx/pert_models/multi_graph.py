...
# Fallback scatter implementation using native PyTorch
def scatter_add(src, index, dim=-1, out=None, dim_size=None):
    """
    Fallback implementation of scatter_add using native PyTorch operations.
    This avoids the need for torch_scatter which has version compatibility issues.
    """
    if out is None:
        if dim_size is None:
    ...
    # Use index_add_ which is the native PyTorch equivalent
        ...
        for l_id in range(self.n_layers):
            ...
            try:
                ...
                if "edge_weights" in locals():
                    del edge_weights
                # Only use CUDA memory management if CUDA is available
