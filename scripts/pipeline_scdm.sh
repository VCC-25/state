...
uv run state tx train \
  data._target_=scdm.SCDatasetPerturbationModule \
  data.toml_config_path="${TOML_CONFIG}" \
  data.embed_key="X_hvg" \
  data.pert_col="target_gene" \
  data.batch_col="batch_var" \
  data.cell_type_key="cell_type" \
